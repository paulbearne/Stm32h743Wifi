 ...
 * changed as the external interrupt is used for other tasks and not just the wifi
 * @param GPIO_Pin: Specifies the pins connected EXTI line
 * @retval None
 */
void wifi_gpio_extint_callback(uint16_t GPIO_Pin)
{
  if ((GPIO_Pin == WIFI_DATA_READY_Pin) ||
      (GPIO_Pin == WIFI_HANDSHAKE_Pin))
  {
...
static void check_and_execute_spi_transaction(void)
{
  ...
  gpio_handshake = HAL_GPIO_ReadPin(WIFI_HANDSHAKE_GPIO_Port,
                                    WIFI_HANDSHAKE_Pin);

  /* data ready line SET -> slave wants to send something */
  gpio_rx_data_ready = HAL_GPIO_ReadPin(WIFI_DATA_READY_GPIO_Port,
                                        WIFI_DATA_READY_Pin);
