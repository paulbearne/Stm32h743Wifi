...
uint8_t W25Qx_Read(uint8_t *pData, uint32_t ReadAddr, uint32_t Size) {
	...
	HAL_SPI_Transmit(&hspi1, cmd, 4, W25QXXXX_TIMEOUT_VALUE);
	/* Reception of the data */
	if (HAL_SPI_Receive(&hspi1, pData, Size, W25QXXXX_TIMEOUT_VALUE)
			!= HAL_OK) {
		return W25Qx_ERROR;
	}
	W25Qx_Disable();
...
uint8_t W25Qx_ReadPage(uint8_t *page, uint16_t pagenum) {
//	uint8_t pData[255] = {0};
	//uint8_t cmd[5] = { READ_CMD, (uint8_t) (pagenum >> 8), (uint8_t) (pagenum
	//		& 0x00FF), 0x00, 0x00 };
  //  uint32_t Size = (page * 255);
	//W25Qx_Enable();

	//HAL_SPI_Transmit(&hspi1, cmd, 5, W25QXXXX_TIMEOUT_VALUE);
	...
	//}
	//W25Qx_Disable();
	return W25Qx_Read(page,pagenum * 255,255);
}
...
void W25Qx_WritePage(uint8_t *page, uint16_t pagenum) {
	W25Qx_Write(page,pagenum * 255,255);
