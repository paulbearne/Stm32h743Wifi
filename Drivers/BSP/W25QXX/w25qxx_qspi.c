...
uint32_t QSPI_EnterFourBytesAddress(QSPI_HandleTypeDef *hqspi);

static uint8_t QSPI_Send_CMD(QSPI_HandleTypeDef *hqspi, uint32_t instruction,
		uint32_t address, uint32_t addressSize, uint32_t dummyCycles,
		uint32_t instructionMode, uint32_t addressMode, uint32_t dataMode,
		uint32_t dataSize);
...
uint16_t w25qxx_GetID(void) {
	...
	if (w25qxx_Mode == w25qxx_SPIMode)
		QSPI_Send_CMD(&hqspi, W25X_QUAD_ManufactDeviceID, 0x00,
		QSPI_ADDRESS_24_BITS, 6, QSPI_INSTRUCTION_1_LINE,
		QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, sizeof(ID));
	else
		QSPI_Send_CMD(&hqspi, W25X_ManufactDeviceID, 0x00, QSPI_ADDRESS_24_BITS,
				0, QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_4_LINES,
				QSPI_DATA_4_LINES, sizeof(ID));

	/* Reception of the data */
	if (HAL_QSPI_Receive(&hqspi, ID, HAL_QPSI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return w25qxx_ERROR;
	}
...
uint8_t w25qxx_ReadSR(uint8_t SR) {
	uint8_t byte = 0;
	if (w25qxx_Mode == w25qxx_SPIMode)
		QSPI_Send_CMD(&hqspi, SR, 0x00, QSPI_ADDRESS_8_BITS, 0,
		QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_NONE, QSPI_DATA_1_LINE, 1);
	else
		QSPI_Send_CMD(&hqspi, SR, 0x00, QSPI_ADDRESS_8_BITS, 0,
		QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_NONE, QSPI_DATA_4_LINES, 1);

	if (HAL_QSPI_Receive(&hqspi, &byte, HAL_QPSI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
...
uint8_t w25qxx_SetReadParameters(uint8_t DummyClock, uint8_t WrapLenth) {
	...
	QSPI_Send_CMD(&hqspi, W25X_SetReadParam, 0x00, QSPI_ADDRESS_8_BITS, 0,
	QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_NONE, QSPI_DATA_4_LINES, 1);
...
uint8_t w25qxx_EraseSector(uint32_t SectorAddress) {
	...
	if (w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi, W25X_SectorErase, SectorAddress,
		QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_1_LINE,
		QSPI_ADDRESS_1_LINE, QSPI_DATA_NONE, 0);
	else
		result = QSPI_Send_CMD(&hqspi, W25X_SectorErase, SectorAddress,
		QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES,
...
uint8_t w25qxx_EraseBlock(uint32_t BlockAddress) {
	...
	if (w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi, W25X_BlockErase, BlockAddress,
		QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_1_LINE,
		QSPI_ADDRESS_1_LINE, QSPI_DATA_NONE, 0);
	else
		result = QSPI_Send_CMD(&hqspi, W25X_BlockErase, BlockAddress,
...
uint8_t w25qxx_EraseChip(void) {
	...
	if (w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi, W25X_ChipErase, 0x00,
		QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_1_LINE,
		QSPI_ADDRESS_NONE, QSPI_DATA_NONE, 0);
	else
		result = QSPI_Send_CMD(&hqspi, W25X_ChipErase, 0x00,
		QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_4_LINES,
		QSPI_ADDRESS_NONE, QSPI_DATA_NONE, 0);
...
uint8_t w25qxx_PageProgram(uint8_t *pData, uint32_t WriteAddr, uint32_t Size) {
	...
	if (w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi, W25X_QUAD_INPUT_PAGE_PROG_CMD, WriteAddr,
		QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_1_LINE,
		QSPI_ADDRESS_1_LINE, QSPI_DATA_4_LINES, Size);
	else
		result = QSPI_Send_CMD(&hqspi, W25X_PageProgram, WriteAddr,
		QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES,
		QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, Size);

	if (result == w25qxx_OK)
		result = HAL_QSPI_Transmit(&hqspi, pData,
...
uint8_t w25qxx_Read(uint8_t *pData, uint32_t ReadAddr, uint32_t Size) {
	...
	result = HAL_QSPI_Command(&hqspi, &s_command,
	HAL_QPSI_TIMEOUT_DEFAULT_VALUE);

	if (result == w25qxx_OK)
		result = HAL_QSPI_Receive(&hqspi, pData,
		HAL_QPSI_TIMEOUT_DEFAULT_VALUE);

	return result;
}

uint8_t w25qxx_ReadPage(uint8_t *page, uint16_t pagenum) {
	QSPI_CommandTypeDef s_command;
	uint8_t result;
	uint8_t pData[255] = {0};
	//uint8_t cmd[5] = { READ_CMD, (uint8_t) (pagenum >> 8), (uint8_t) (pagenum
	//	& 0x00FF), 0x00, 0x00 };
	...
	// page read address * 255 for actual address
	s_command.Address = *page * 255;
	s_command.AddressMode = QSPI_ADDRESS_4_LINES;
	s_command.AddressSize = QSPI_ADDRESS_24_BITS;
	s_command.AlternateByteMode = QSPI_ALTERNATE_BYTES_4_LINES;
	s_command.AlternateBytes = 0xFF;
...
void w25qxx_WriteNoCheck(uint8_t *pBuffer, uint32_t WriteAddr,
...
static uint8_t QSPI_Send_CMD(QSPI_HandleTypeDef *hqspi, uint32_t instruction,
	...
	if (HAL_QSPI_Command(hqspi, &Cmdhandler, HAL_QPSI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK)
		return w25qxx_ERROR;
...
static uint8_t QSPI_EnterQPI(QSPI_HandleTypeDef *hqspi) {
	...
	QSPI_Send_CMD(hqspi, W25X_EnterQSPIMode, 0x00, QSPI_ADDRESS_8_BITS, 0,
	QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_NONE, QSPI_DATA_NONE, 0);
