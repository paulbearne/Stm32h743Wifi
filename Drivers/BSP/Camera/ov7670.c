...
void OV7670_Light_Mode(uint8_t mode)
{
	...
	OV7670_WriteReg(REG_COM8, &reg13val);
	OV7670_WriteReg(REG_BLUE,&reg01val);
	OV7670_WriteReg(REG_RED,&reg02val);
}				
...
void OV7670_Color_Saturation(uint8_t sat)
{
	...
	OV7670_WriteReg(MTX1,&reg4f5054val);
	OV7670_WriteReg(MTX2,&reg4f5054val);
	OV7670_WriteReg(MTX3,0X00);
	OV7670_WriteReg(MTX4,&reg52val);
	OV7670_WriteReg(MTX5,&reg53val);
	OV7670_WriteReg(MTX6,&reg4f5054val);
	OV7670_WriteReg(MTXS,&reg54val);
}
...
void OV7670_Brightness(uint8_t bright)
{
	...
	OV7670_WriteReg(REG_BRIGHT,&reg55val);
}
...
void OV7670_Contrast(uint8_t contrast)
{
	...
	OV7670_WriteReg(REG_CONTRAS,&reg56val);
}
...
void OV7670_Special_Effects(uint8_t eft)
{
	...
	OV7670_WriteReg(REG_TSLB,&reg3aval);
	OV7670_WriteReg(0X68,&reg67val);
	OV7670_WriteReg(0X67,&reg68val);
}
...
void ov7670_Window_Set(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height)
{
	...
	OV7670_ReadReg(0X03,&temp);
	temp&=0XF0;
	temp|=((endx&0X03)<<2)|(sx&0X03);
	OV7670_WriteReg(0X03,&temp);
	sx = (sx >>2)&0xFF;
	endx = (endx >>2)&0xFF;
	OV7670_WriteReg(0X19,(uint8_t *)&sx);
	OV7670_WriteReg(0X1A,(uint8_t *)&endx);

	OV7670_ReadReg(0X32,&temp);
	temp&=0XC0;
	temp|=((endy&0X07)<<3)|(sy&0X07);
	OV7670_WriteReg(0X32,&temp);
	sy = (sy>>3)&0xFF;
	OV7670_WriteReg(0X17,(uint8_t *)&sy);
	endy = (endy>>3)&0xFF;
	OV7670_WriteReg(0X18,(uint8_t *)&endy);
}
